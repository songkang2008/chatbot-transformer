...
def get_tokenizer(MAX_SEQ_LENGTH, BATCH_SIZE):
    ...
    with open('./deal_data_train.txt', encoding='utf-8') as f:
        lines = f.readlines()
        print(lines[0])
        word_pairs = [[w for w in l.split('\t')] for l in lines[:8000]]
        train_examples = zip(*word_pairs)
    ...
    def encode(input, seq_length=MAX_SEQ_LENGTH):
        tokens_input = tokenizer_zh.tokenize(tf.compat.as_text(input))
        lang1 = tokenizer_zh.convert_tokens_to_ids(['[CLS]'] + tokens_input + ['[SEP]'])
        if len(lang1) < seq_length:
            lang1 = lang1 + list(np.zeros(seq_length - len(lang1), 'int32'))
        elif len(lang1)> seq_length:
            lang1 = lang1[:seq_length]
        #
        # # inser SOS and EOS
        # #         lang2 = [tokenizer_en.vocab_size] + tokenizer_en.encode(
        # #             tf.compat.as_text(en.numpy())) + [tokenizer_en.vocab_size + 1]t
        # tokens_ans = tokenizer_zh.tokenize(tf.compat.as_text(ans))
        # lang2 = tokenizer_zh.convert_tokens_to_ids(['[CLS]'] + tokens_ans + ['[SEP]'])
    ...
    input_lang, target_lang = train_examples
    # len_d =[]
    # for target_langs in target_lang:
    #     len_d.append(len(target_langs))
    # max = tf.reduce_max(len_d)
    # print('len_max', max)


    print(input_lang[1])
    print(target_lang[1])
    input_tensor = tf.convert_to_tensor(list(map(encode, input_lang)))
    print('input_tensor', input_tensor, input_tensor.shape)
    target_tensor = tf.convert_to_tensor(list(map(encode, target_lang)))
    print('target_tensor', target_tensor, target_tensor.shape)
    ...
    return train_dataset, tokenizer_zh


if __name__ == '__main__':
    get_tokenizer(100, 128)